...
This catalogue lists the named coaching systems we intend to document as philosophy profiles. The summaries are orientation only: they do not make a system active, define card content, or replace the detailed interpretation and source record inside that system's folder.

Every system below is currently **planned**. A system becomes active only after its `philosophy.md` and `sources.md` are developed and reviewed against the shared coaching foundation.
